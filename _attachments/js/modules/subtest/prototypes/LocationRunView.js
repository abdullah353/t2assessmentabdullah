...
LocationRunView = (function(_super) {
  ...
  LocationRunView.prototype.initialize = function(options) {
    ...
    for (i = 0, _len = _ref.length; i < _len; i++) {
      ...
      for (_i = 0, _len2 = location.length; _i < _len2; _i++) {
        ...
        this.haystack[i].push(locationData);
      }
    }
  ...
  LocationRunView.prototype.showOptions = function(event) {
    var atLeastOne, field, html, i, isThere, j, needle, otherField, result, results, stack, _i, _len, _len2, _len3, _len4, _ref, _ref2, _ref3,a,ha;
    ...
    if (atLeastOne) {
      ...
      for (_i = 0, _len4 = results.length; _i < _len4; _i++) {
       ...
       ha = $(location).attr('hash');
       if(localStorage[ha]){
        a = localStorage[ha];
        a = a.split('|');
        _.each(a,function(item){
          item = item.replace("undefined",'');
          html = html.replace(item,"<del>"+item+"</del>");
        });
       }
