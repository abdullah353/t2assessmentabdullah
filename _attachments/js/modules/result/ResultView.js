...
ResultView = (function(_super) {
  ...
  ResultView.prototype.save = function() {
    ...
    if (this.model.save()) {
      ...
      this.$el.find('.confirmation').removeClass('confirmation');
    } else {
      Utils.midAlert("Save error");
      this.$el.find('.save_status').html("Results may not have saved");
    }
    window.allResult = new Results;
    return allResult.fetch({
      success: function() {
        return window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, gotFS);
      }
    });
  };
